...
fLoadTOF <- function(spect.dir, spect.name) {
  ## 2. load spectra file generated by the Leicester PTR-TOF-MS
  ## instrument
  ##
...
fNormTOF <- function(spect.df, ref.mz, scan.ini, scan.fin) {
  ## 3. normalize PTR-TOF-MS spectra to 1 million counts of a
  ## reference ion (e.g., [H3O]+); calculate average spectrum and
  ## standard deviation of a group of normalized scans
  ##
...
fPlotSpect <- function(spect.df, mz.min, mz.max, sc.min, sc.max, fn.str) {
  ## 4. rotate data.frame of spectra; make plots of time spectra (ion
...
fLoadCIMS <- function(cims.dir, cims.fn) {
  ## 5. load data file generated by the Leicester CIMS instrument
  ##
  ## input:
...
fSpectraCIMS <- function(cims.dir, cims.lst) {
  ## 6. load spectra generated by the Leicester CIMS instrument and
